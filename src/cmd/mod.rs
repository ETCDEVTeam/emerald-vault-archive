...
use super::emerald::storage::{default_path, StorageController, KeyfileStorage};
...
impl<'a> CmdExecutor<'a> {
    ...
    pub fn new(matches: ArgMatches<'a>) -> Result<Self, Error> {
        ...
        let sec_level = matches
            .value_of("security-level").and_then(|s| KdfDepthLevel::from_str(s).ok())
            .unwrap_or(KdfDepthLevel::default());
        info!("Security level: `normal`");

        let mut base_path = PathBuf::new();
        if let Some(p) = matches.value_of("base-path").or(env.emerald_base_path) {
            base_path.push(&p)
        } else {
